...
function bindSearchEvents() {
  ...
  if (searchBtn) searchBtn.addEventListener('click', () => { searchKeyword = document.getElementById('searchInput').value.trim(); renderJobs(); });
  if (searchInput) searchInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') searchBtn.click(); });
}

document.addEventListener('DOMContentLoaded', () => {
  initJobsData();
  if (document.getElementById('categoriesContainer')) { renderCategories(); renderJobs(); bindSearchEvents(); }
  const btnOwner = document.getElementById('btnOwner'), btnAdmin = document.getElementById('btnAdmin'), btnUser = document.getElementById('btnUser');
